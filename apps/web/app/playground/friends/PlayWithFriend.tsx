...
import { useSearchParams } from "next/navigation";  // ✅ use directly in client
...
const PlayWithFriend = (props: Props) => {
  ...
  const quizId = searchParams.get("quiz");  // ✅ this now works perfectly
...
const ProfileComponent = ({ name, avatarUrl }: { name: string; avatarUrl: string }) => {
  return (
    <div className="flex flex-col items-center">
      <img
        className="w-36 md:w-64 rounded-full p-1 outline-secondary outline-4"
